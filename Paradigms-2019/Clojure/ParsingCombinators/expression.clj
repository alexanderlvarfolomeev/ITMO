...
(declare *valueInfix, *valueInfixPartial)
(defn *xxx [p sign] (+map (partial reduce #(list (first %2) %1 (second %2))) (+seqf cons *ws p (+star (+seq *ws sign *ws p)) *ws)))
...
(defn *divmul [] (*xxx (+or (delay (*listInfix)) (delay (*func)) (delay *number) (delay *variable)) *divmulSymbol))
(defn *addsub [] (*xxx (delay (*valueInfixPartial)) *addsubSymbol))
