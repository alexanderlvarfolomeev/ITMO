...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        Method before = getMethod(pageClass, "before");
        Method method = getMethod(pageClass, route.getAction());
        Method after = getMethod(pageClass, "after");
        ...
        try {
            ...
            Object[] args = Arrays.stream(method.getParameterTypes()).map(p -> {
                if (p.equals(Map.class)) {
                    return view;
                }
                if (p.equals(HttpServletRequest.class)) {
                    return request;
                }
                return null;
            }).toArray();
    ...
    private Method getMethod(Class<?> pageClass, String action) throws NotFoundException {
        Method method = null;
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                method = Arrays.stream(clazz.getDeclaredMethods())
                        .filter(m -> m.getName().equals(action)).findFirst().get();
