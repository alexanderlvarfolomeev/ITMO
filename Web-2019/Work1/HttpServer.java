...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        String redirectedUri = URLDecoder.decode(request.getUri(), "utf8");
        File file = new File(root, redirectedUri);
        if (file.isDirectory()) {
            redirectedUri = (redirectedUri.endsWith("/") ? "" : "/") + "index.html";
            file = new File(file, redirectedUri);
        }
        if (file.isFile()) {
            byte[] body = readFile(file);
            if (Integer.toHexString(Arrays.hashCode(body)).equals(request.getHeader("If-None-Match"))) {
            ...
            response.setHeader("ETag", Integer.toHexString(Arrays.hashCode(body)));
